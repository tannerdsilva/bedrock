...
#ifndef __CLIBBEDROCK_FIFO_H
#define __CLIBBEDROCK_FIFO_H
...
typedef struct __cbedrock_fifo_linkpair {
	...
	_Atomic __cbedrock_optr_t ____cp;
...
} __cbedrock_fifo_linkpair_t;

/// defines a non-null pointer to a fifo pair structure, facilitating operations on the entire chain.
typedef __cbedrock_fifo_linkpair_t*_Nonnull __cbedrock_fifo_linkpair_ptr_t;
...
/// @return a heap pointer to a newly initialized `_cswiftslash_fifo_linkpair_t`. NOTE: this pointer must be closed with `__cbedrock_fifo_close` to free all associated memory.

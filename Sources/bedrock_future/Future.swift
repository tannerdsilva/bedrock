...
public final class Future<Produced, Failure>:@unchecked Sendable where Failure:Swift.Error {
	...
	internal let prim:UnsafeMutablePointer<__cbedrock_future_t>
	...
	deinit {
		...
		withUnsafeMutablePointer(to:&deallocResult) { rptr in
			__cbedrock_future_t_destroy(prim, rptr, futureSyncResultHandler, futureSyncErrorHandler)
		}
...
extension Future {
	...
	public func setSuccess(_ result:consuming Produced) throws(InvalidFutureStateError) {
		...
		guard __cbedrock_future_t_broadcast_res_val(prim, 1, op) else {
	...
	public func setFailure(_ error:consuming Failure) throws(InvalidFutureStateError) {
		...
		guard __cbedrock_future_t_broadcast_res_throw(prim, 1, op) else {
	...
	public func cancel() throws(InvalidFutureStateError) {
		guard __cbedrock_future_t_broadcast_cancel(prim) else {
			throw InvalidFutureStateError()
		}
	}
}
...
extension Future {
	...
	public func result(throwingOnCurrentTaskCancellation taskThrowType:Never.Type = Never.self) async -> Result<Produced, Failure>? {
		var syncResult = SyncResult()
		var memory = __cbedrock_future_wait_t_init_struct()
		return await _result_main(throwing:Never.self, onCurrentTaskCancellation:fatalError("bedrock_future :: caught trying to create an error to throw within Never type. this is an internal error. \(#file):\(#line)"), memory:&memory, syncResult:&syncResult)
	}
	...
	@discardableResult public func whenResult(_ callback:consuming @escaping @Sendable (Result<Produced, Failure>?) -> Void) -> UInt64? {
		...
		let waitID = __cbedrock_future_t_wait_async(prim, arPtr, futureAsyncResultHandler, futureAsyncErrorHandler, futureAsyncCancelHandler)
		if waitID == 0 {
			return nil
		} else {
			return waitID
	...
	@discardableResult public func cancelWaiter(_ waiterID:consuming UInt64) -> Bool {
		return __cbedrock_future_t_wait_async_invalidate(prim, waiterID)
	}
}

extension Future {
	fileprivate borrowing func _result_main<E>(throwing _:E.Type, onCurrentTaskCancellation throwOnTaskCancellation:@autoclosure () -> E, memory:UnsafeMutablePointer<__cbedrock_future_wait_t>, syncResult sPtr:UnsafeMutablePointer<SyncResult>) async throws(E) -> Result<Produced, Failure>? where E:Swift.Error {
		return try await sPtr.pointee.withWaiterPrimitiveAccess { wptr throws(E) -> Result<Produced, Failure>? in
			let waiterPtr = __cbedrock_future_t_wait_sync_register(prim, sPtr, futureSyncResultHandler, futureSyncErrorHandler, futureSyncCancelHandler, memory)
			if waiterPtr == nil {
				...
				if E.self == Never.self {
					...
					await withUnsafeContinuation({ (cont:UnsafeContinuation<Void, Never>) in
						__cbedrock_future_t_wait_sync_block(prim, waiterPtr!)
						cont.resume()
					})
					...
					let cancelID = __cbedrock_future_wait_id_get(waiterPtr!)
					...
					await withTaskCancellationHandler {
						...
						__cbedrock_future_wait_sync_invalidate(prim, cancelID)
					}

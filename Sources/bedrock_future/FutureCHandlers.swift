...
internal let futureSyncResultHandler:__cbedrock_future_result_val_handler_f = { resType, resPtr, ctxPtr in
	ctxPtr!.assumingMemoryBound(to:SyncResult.self).pointee.setResult(type:resType, pointer:resPtr)
}
/// the sync handler for errors
internal let futureSyncErrorHandler:__cbedrock_future_result_err_handler_f = { errType, errPtr, ctxPtr in
...
internal let futureSyncCancelHandler:__cbedrock_future_result_cncl_handler_f = { ctxPtr in
	ctxPtr!.assumingMemoryBound(to:SyncResult.self).pointee.setCancel()
}

/// the async handler for results
internal let futureAsyncResultHandler:__cbedrock_future_result_val_handler_f = { resType, resPtr, ctxPtr in
...
internal let futureAsyncErrorHandler:__cbedrock_future_result_err_handler_f = { errType, errPtr, ctxPtr in
	let boundPtr = ctxPtr!.assumingMemoryBound(to:AsyncResult.self)
...
internal let futureAsyncCancelHandler:__cbedrock_future_result_cncl_handler_f = { ctxPtr in
	let boundPtr = ctxPtr?.assumingMemoryBound(to:AsyncResult.self)

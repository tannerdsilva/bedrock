...
import __cbedrock_identified_list
...
public final class IdentifiedList<Element>:@unchecked Sendable {
	...
	private let list_store_ptr:UnsafeMutablePointer<__cbedrock_identified_list_pair_t>
	
	deinit {
		var allPtrs = [UnsafeMutableRawPointer]()
		withUnsafeMutablePointer(to: &allPtrs) { allPtrsPtr in
			__cbedrock_identified_list_close(list_store_ptr, { key, ptr, ctx in
	...
	public init() {
		list_store_ptr = UnsafeMutablePointer<__cbedrock_identified_list_pair_t>.allocate(capacity:1)
		list_store_ptr.initialize(to:__cbedrock_identified_list_init())
	}
	...
	public func forEach(_ body:(UInt64, Element) -> Void) {
		...
		withUnsafeMutablePointer(to: &allPtrs) { allPtrsPtr in
			__cbedrock_identified_list_iterate(list_store_ptr, { key, ptr, ctx in
		...
		__cbedrock_identified_list_iterate_hanginglock_complete(list_store_ptr)
	}
	...
	public func insert(_ data:consuming Element) -> UInt64 {
		return __cbedrock_identified_list_insert(list_store_ptr, Unmanaged.passRetained(Contained(data)).toOpaque())
	}
	...
	@discardableResult public func remove(_ key:UInt64) -> Element? {
		switch (__cbedrock_identified_list_remove(list_store_ptr, key)) {

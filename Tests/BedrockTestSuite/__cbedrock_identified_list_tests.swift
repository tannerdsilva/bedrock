...
import Testing
@testable import __cbedrock_identified_list

extension Tag {
	@Tag internal static var __cbedrock_identified_list:Self
}

extension __cbedrock_test_suite {
	@Suite("__cbedrock_identified_list",
		.serialized,
		.tags(.__cbedrock_identified_list)
	)
	internal struct IdentifiedList {
		...
		fileprivate final class IdentifiedListHarness:@unchecked Sendable {
			fileprivate let listPtr:UnsafeMutablePointer<__cbedrock_identified_list_pair_t>
			
			/// initializes a new atomic list instance.
			fileprivate init() {
				listPtr = UnsafeMutablePointer<__cbedrock_identified_list_pair_t>.allocate(capacity:1)
				listPtr.initialize(to:__cbedrock_identified_list_init())
			}
			...
			fileprivate func iterate(hang:Bool = false, _ consumer:@escaping (UInt64, UnsafeMutableRawPointer?) -> Void) {
				...
				__cbedrock_identified_list_iterate(listPtr, IdentifiedListHarness.consumerFunction, cc, false)
				_ = Unmanaged<ConsumerContext>.fromOpaque(cc).takeRetainedValue()
			}

			fileprivate func closeHangingLock() {
				__cbedrock_identified_list_iterate_hanginglock_complete(listPtr)
			}
			...
			fileprivate func close(consumer:@escaping ((UInt64, UnsafeMutableRawPointer?) -> Void) = { _, _ in }) {
				let cc = Unmanaged.passRetained(ConsumerContext(consumer)).toOpaque()
				__cbedrock_identified_list_close(listPtr, IdentifiedListHarness.consumerFunction, cc)
				_ = Unmanaged<ConsumerContext>.fromOpaque(cc).takeRetainedValue()
			}
			...
			/// C function pointer compatible with __cbedrock_identified_list_ptr_f
			private static let consumerFunction:__cbedrock_identified_list_iterator_f = { (key, ptr, ctx) in
		...
		// MARK: - test cases
		@Test("__cbedrock_identified_list :: initialization and deallocation", .timeLimit(.minutes(1)))
		func testAtomicListInitialization() {
			list.close()
		}

		@Test("__cbedrock_identified_list :: insert and remove single element", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: insert multiple elements", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: close list with consumer function", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: remove non-existent key", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: concurrent insertions and removals", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: fuzz testing", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: iterate over elements", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: stress test with large number of elements", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_identified_list :: insert and remove same element multiple times", .timeLimit(.minutes(1)))

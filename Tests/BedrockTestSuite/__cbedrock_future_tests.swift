...
import Testing
@testable import __cbedrock_future

import Synchronization

extension Tag {
	@Tag internal static var __cbedrock_future:Self
}

extension __cbedrock_test_suite {
	@Suite("__cbedrock_future",
		.serialized,
		.tags(.__cbedrock_future)
	)
	internal struct __cbedrock_future_tests {
		// MARK: c harness
		fileprivate final class Harness:@unchecked Sendable {
			...
			private let futurePtr:UnsafeMutablePointer<__cbedrock_future_t>
			fileprivate init() {
				self.futurePtr = __cbedrock_future_t_init()
			}

			fileprivate func cancel() -> Bool {
				__cbedrock_future_t_broadcast_cancel(futurePtr)
			}
			...
			fileprivate func waitAsync() async throws -> Result? {
				...
				let keyThing = __cbedrock_future_t_wait_async(self.futurePtr, asyncResultMemory, Self.futureAsyncResultHandler, Self.futureAsyncErrorHandler, Self.futureAsyncCancelHandler)
				try await withTaskCancellationHandler(operation: { 
					...
					__cbedrock_future_t_wait_async_invalidate(futurePtr, keyThing)
				})
				return resultStore!.1
			}
			internal func broadcastResultValue(resType: UInt8, resVal:UnsafeMutableRawPointer?) -> Bool {
				return __cbedrock_future_t_broadcast_res_val(futurePtr, resType, resVal)
			}
			internal func broadcastErrorValue(errType: UInt8, errVal:UnsafeMutableRawPointer?) -> Bool {
				return __cbedrock_future_t_broadcast_res_throw(futurePtr, errType, errVal)
			}
			...
			private final class AsyncResult {
				...
				fileprivate func setResult(type:UInt8, pointer result:__cbedrock_optr_t?) {
				...
				fileprivate func setError(type:UInt8, pointer error:__cbedrock_optr_t?) {
					pthread_mutex_lock(&istate)
			...
			private static let futureAsyncResultHandler:__cbedrock_future_result_val_handler_f = { resType, resPtr, ctxPtr in
				ctxPtr?.assumingMemoryBound(to:AsyncResult.self).pointee.setResult(type:resType, pointer:resPtr)
			}
			private static let futureAsyncErrorHandler:__cbedrock_future_result_err_handler_f = { errType, errPtr, ctxPtr in
				ctxPtr?.assumingMemoryBound(to:AsyncResult.self).pointee.setError(type:errType, pointer:errPtr)
			}
			private static let futureAsyncCancelHandler:__cbedrock_future_result_cncl_handler_f = { ctxPtr in
			...
			private struct SyncResult {
				...
				fileprivate init() {}
				fileprivate mutating func setResult(type:UInt8, pointer resultPtr:__cbedrock_optr_t?) {
					ron = .result(.success(type, resultPtr))
					assPT = pthread_self()
				}
				fileprivate mutating func setError(type:UInt8, pointer errorPtr:__cbedrock_optr_t?) {
				...
				try await withUnsafeThrowingContinuation { (cont:UnsafeContinuation<Result?, Swift.Error>) in
					do {
						...
						withUnsafeMutablePointer(to:&srInstance) { srInstance in
							...
							let ptr = __cbedrock_future_t_wait_sync_register(futurePtr, srInstance, Self.futureSyncResultHandler, Self.futureSyncErrorHandler, Self.futureSyncCancelHandler, &memory)
							guard ptr != nil else {
								return
							}
							__cbedrock_future_t_wait_sync_block(futurePtr, ptr!)
						}
			...
			private static let futureSyncResultHandler:__cbedrock_future_result_val_handler_f = { resType, resPtr, ctxPtr in
			...
			private static let futureSyncErrorHandler:__cbedrock_future_result_err_handler_f = { errType, errPtr, ctxPtr in
			...
			private static let futureSyncCancelHandler:__cbedrock_future_result_cncl_handler_f = { ctxPtr in
		...
		@Suite("__cbedrock_future core (sync)",
			.serialized
		)
		struct CoreSync {
			private var future = Harness()

			@Test("__cbedrock_future :: core :: sync result", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: core :: sync error", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: core :: sync cancel", .timeLimit(.minutes(1)))
			func testSyncWaiterCancellation() async throws {
				try await withThrowingTaskGroup(of:Harness.Result?.self) { tg in
					tg.addTask {
		...
		@Suite("__cbedrock_future core (async)",
			.serialized
		)
		struct CoreAsync {
			private var future = Harness()
			...
			@Test("__cbedrock_future :: core :: async result", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: core :: async waiter invalidation", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: core :: async error", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: core :: async cancel", .timeLimit(.minutes(1)))
		...
		@Suite("__cbedrock_future (additional coverage)",
			.serialized
		)
		struct Additional {
			private var future = Harness()

			@Test("__cbedrock_future :: broadcast result multiple times", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: broadcast error after result", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: broadcast error multiple times", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: broadcast result after error", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: fuzz testing", .timeLimit(.minutes(1)))
		...
		@Suite("__cbedrock_future (multiple waiters)",
			.serialized
		)
		struct MultipleWaiters {
			@Test("__cbedrock_future :: multiple waiters :: async result", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: multiple waiters :: async error", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: multiple waiters :: broadcast before wait", .timeLimit(.minutes(1)))
			...
			@Test("__cbedrock_future :: multiple waiters :: error and result race", .timeLimit(.minutes(1)))

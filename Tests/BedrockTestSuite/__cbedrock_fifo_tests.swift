...
import Testing
@testable import __cbedrock_fifo

import class Foundation.ProcessInfo

extension Tag {
	@Tag internal static var __cbedrock_fifo:Self
}

extension __cbedrock_test_suite {
	@Suite("__cbedrock_fifo",
		.serialized,
		.tags(.__cbedrock_fifo)
	)
	internal struct __cbedrock_fifo_tests {
		// MARK: c harness
		private final class Harness:@unchecked Sendable {
			private let fifoPtr:UnsafeMutablePointer<__cbedrock_fifo_linkpair_t>
			fileprivate init(hasMutex:Bool = true) {
				if hasMutex {
					fifoPtr = __cbedrock_fifo_init(true)
				} else {
					fifoPtr = __cbedrock_fifo_init(false)
				}
			}
			fileprivate func pass(_ data: UnsafeMutableRawPointer) -> Int8 {
				return __cbedrock_fifo_pass(fifoPtr, data)
			}
			/// consumes data from the FIFO in a non-blocking manner
			fileprivate func consumeNonBlocking() -> (__cbedrock_fifo_consume_result_t, UnsafeMutableRawPointer?) {
				var consumedData:UnsafeMutableRawPointer?
				let result = __cbedrock_fifo_consume_nonblocking(fifoPtr, &consumedData)
				return (result, consumedData)
			}
			/// consumes data from the FIFO in a blocking manner
			fileprivate func consumeBlocking() async -> (__cbedrock_fifo_consume_result_t, UnsafeMutableRawPointer?) {
				return await withUnsafeContinuation { (continuation:UnsafeContinuation<(__cbedrock_fifo_consume_result_t, UnsafeMutableRawPointer?), Never>) in
					var consumedData:UnsafeMutableRawPointer?
					continuation.resume(returning:(__cbedrock_fifo_consume_blocking(fifoPtr, &consumedData), consumedData))
				}
			}
			/// caps the FIFO with a final element
			fileprivate func passCap(_ capData: UnsafeMutableRawPointer?) -> Bool {
				return __cbedrock_fifo_pass_cap(fifoPtr, capData)
			}
			/// sets the maximum number of elements in the FIFO
			fileprivate func setMaxElements(_ maxElements: size_t) -> Bool {
				return __cbedrock_fifo_set_max_elements(fifoPtr, maxElements)
			}
			private func closeFIFO() -> (Bool, UnsafeMutableRawPointer?) {
				var cptr:UnsafeMutableRawPointer? = nil
				return (__cbedrock_fifo_close(fifoPtr, { _, _ in }, nil, &cptr), cptr)
			}
			deinit {
				_ = closeFIFO()
			}
		...
		@Test("__cbedrock_fifo :: basic init and deinit", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_fifo :: nonblocking consume from empty fifo", .timeLimit(.minutes(1)))
		func consumeFromEmpty() {
			let (consumeResult, consumedData) = fifo!.consumeNonBlocking()
			#expect(consumeResult == __CBEDROCK_FIFO_CONSUME_WOULDBLOCK)
			#expect(consumedData == nil)
		}

		@Test("__cbedrock_fifo :: pass then consume (single element - nonblocking)", .timeLimit(.minutes(1)))
		func passThenConsumeNBSimpleSingle() async {
			...
			#expect(consumeResult2 == __CBEDROCK_FIFO_CONSUME_RESULT)
			#expect(consumedData2 == data)
		}

		@Test("__cbedrock_fifo :: pass then consume (single element - blocking)", .timeLimit(.minutes(1)))
		func passThenConsumeBSingle() async {
			...
			#expect(consumedResult2 == __CBEDROCK_FIFO_CONSUME_RESULT)
			#expect(consumedData2 == data)
		}

		static let fifoConsecutiveCount = 10

		@Test("__cbedrock_fifo :: pass then consume (consecutive elements - nonblocking)", .timeLimit(.minutes(1)))
		func passThenConsumeSimpleMultiple() async {		
			...
			for i in 0..<Self.fifoConsecutiveCount {
				let (consumeResult, consumedData) = fifo!.consumeNonBlocking()
				#expect(consumeResult == __CBEDROCK_FIFO_CONSUME_RESULT)
				#expect(consumedData != nil)
		...
		@Test("__cbedrock_fifo :: pass then consume (consecutive elements - blocking)")
		...
		@Test("__cbedrock_fifo :: set max elements", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_fifo :: fuzz testing FIFO", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_fifo :: blocking consume with cap", .timeLimit(.minutes(1)))
		func testBlockingConsumeWithCap() async {		
			...
			let (consumeResult, consumedData) = await fifo!.consumeBlocking()
			#expect(consumeResult == __CBEDROCK_FIFO_CONSUME_CAP)
			#expect(consumedData == capData)
		}

		@Test("__cbedrock_fifo :: set max elements to zero", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_fifo :: set max elements to 1", .timeLimit(.minutes(1)))
		func testSetMaxElementsToOne() async {
			...
			let (consumeResult2, consumedData2) = fifo!.consumeNonBlocking()
			#expect(consumeResult2 == __CBEDROCK_FIFO_CONSUME_WOULDBLOCK)
			#expect(consumedData2 == nil)
		}

		@Test("__cbedrock_fifo :: set max elements to n, while passing n*2 elements", .timeLimit(.minutes(1)))

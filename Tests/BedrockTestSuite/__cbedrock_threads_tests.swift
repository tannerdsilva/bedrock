...
@testable import __cbedrock_threads
...
extension Tag {
	@Tag internal static var __cbedrock_threads:Self
}

extension __cbedrock_test_suite {
	@Suite("__cbedrock_threads",
		.serialized,
		.tags(.__cbedrock_threads)
	)
	internal struct ThreadTests {
		...
		fileprivate final class ThreadHarness:@unchecked Sendable {
			...
			// thread
			fileprivate var thread:__cbedrock_threads_t_type? = nil
		
			// function pointers for the thread configuration
			fileprivate let alloc_f:__cbedrock_threads_alloc_f!
			fileprivate let run_f:UnsafeMutablePointer<__cbedrock_threads_main_f> = .allocate(capacity:1)
			fileprivate let cancel_f:__cbedrock_threads_cancel_f!
			fileprivate let dealloc_f:__cbedrock_threads_dealloc_f!
			...
			fileprivate func startThread() {
				...
				let config = __cbedrock_threads_config_init(
					arg,
					alloc_f,
					run_f.pointee,
					cancel_f,
				...
				var result:Int32 = 0
				let thread = __cbedrock_threads_config_run(config, &result)
				self.thread = thread
			}
		...
		// MARK: test cases
		@Test("__cbedrock_threads :: sequence (no cancellation)", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_threads :: sequence (immediate cancellation)", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_threads :: deallocate short circuit", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_threads :: deallocate on immediate exit", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_threads :: cancellation after work completion", .timeLimit(.minutes(1)))
		...
		@Test("__cbedrock_threads :: concurrent threads with cancellation", .timeLimit(.minutes(1)))
